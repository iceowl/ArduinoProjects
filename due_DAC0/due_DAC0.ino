...
void loop() {
  ...
  //int t_sample = map(analogRead(A0), 0, 4095, 0, oneHzSample);
...
//  analogWrite(DAC0, waveformsTable[wave0][i]);  // write the selected waveform on DAC0
 ...
 // analogWrite(pwmOut,waveformsTable[wave0][i]);
 double k = 127.0 * sin ((double)i * 3.14159/512.0);
 k += 127.0;
 //Serial.println(k);
 analogWrite(pwmOut, k);
